...
import { getSettingsTime } from "../../util/GetClockTimeSettings";
...
const DisplayTime = (props: {
    ...
    const configTime = getSettingsTime(props.clock);
...
const DisplayDirection = (props: { clock: ClockSource }) => {
...
const DisplayBehaviour = (props: { clock: ClockSource }) => {
    const settings = (props.clock.data() as any).settings;
...
const DisplayOffset = (props: { clock: ClockSource }) => {
    const settings = (props.clock.data() as any).settings;
...
const ClockSettingsComponent = (props: {
    ...
    const [displayName, setDisplayName] = useState(
        props.clock.clock.displayName
    );

    useEffect(() => {
        if (displayName !== props.clock.clock.displayName) {
            const delayChange = setTimeout(() => {
    ...
    return (
        <Panel className={props.className}>
            ...
            <Dialog open={open} onClose={() => setOpen(false)}>
                <DialogTitle>{`Are you sure you want to delete "${props.clock.clock.displayName}"?`}</DialogTitle>
                <DialogActions>
                    <Button
                        onClick={() => {
                            setOpen(false);
            ...
            >
                <DialogTitle>{`Configure ${props.clock.clock.displayName}`}</DialogTitle>
                <DialogContent>
                    <DiagContainer>
